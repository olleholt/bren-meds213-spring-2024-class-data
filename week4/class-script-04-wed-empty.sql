...
SELECT Site, AVG(Snow_cover) FROM Snow_cover Group BY Site;
...
-- remember to DROP VIEW Site_avg_snowcover_nonseroes;
SELECT Site, Site_avg_snowcover_nozeroes.avg_cover - Site_avg_snowcover.avg_cover AS avg_diff
    FROM Site_avg_snowcover
    JOIN Site_avg_snowcover_nozeroes USING (Site);
